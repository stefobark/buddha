...
class UsersController extends \BaseController {
	...
	public function show()
	{
		...
		$test = SphinxQL::raw("select json.id, json.sponsors from rt_u_bills where json['sponsors'][0]['leg_id']='WAL000122'");

		foreach($q as $id){
			$f_id = $id['json.leg_id'];
			
			$l_primary[] = SphinxQL::raw("select json.id, json.sponsors from rt_l_bills where json['sponsors'][0]['leg_id']='$f_id'");
			$u_primary[] = SphinxQL::raw("select json.id, json.sponsors from rt_u_bills where json['sponsors'][0]['leg_id']='$f_id'");
		}
		
		return View::make('users.show', array('user' => $user, 'q' => $q, 'test' => $test, 'l_primary' => $l_primary, 'u_primary' => $u_primary));
	}
