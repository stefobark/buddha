...
class UsersController extends \BaseController {
	...
	public function show()
	{
		...
		$q = SphinxQL::raw("select json.leg_id, json.first_name, json.last_name, json.email, json.office_phone from rt_legs where json.district = $user->leg_district");

		foreach($q as $id){
			$f_id = $id['json.leg_id'];
			
			$l_primary[] = SphinxQL::raw("select json.id AS b_id, json['versions'][0]['name'] AS name, json['title'] AS title, json['versions'][0]['url'] AS url1, json['sources'][0]['url'] AS url2, json['subjects'] AS subjects from rt_l_bills where json['sponsors'][0]['leg_id']='$f_id'");
			$u_primary[] = SphinxQL::raw("select json.id AS b_id, json['versions'][0]['name'] AS name, json['title'] AS title, json['versions'][0]['url'] AS url1, json['sources'][0]['url'] AS url2, json['subjects'] AS subjects from rt_l_bills where json['sponsors'][0]['leg_id']='$f_id'");
		}
		
		
		
		return View::make('users.show', array('user' => $user, 'q' => $q, 'l_primary' => $l_primary, 'u_primary' => $u_primary));
	}
